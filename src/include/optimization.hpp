...
public:
    ...
    nlls_fwi_eco(nl_we_op * L, std::shared_ptr<vecReg<data_t> > m, std::shared_ptr<vecReg<data_t> > d, nloper * P = nullptr, std::shared_ptr<vecReg<data_t> > gmask = nullptr, std::shared_ptr<vecReg<data_t> > w = nullptr, std::shared_ptr<vecReg<data_t> > filter = nullptr){
        ...
        {
            sWeighting * sW = new sWeighting(hypercube<data_t>(_pg->getHyper()->getAxis(1), _pg->getHyper()->getAxis(2), _pg->getHyper()->getAxis(3), axis<data_t>(_L->_par.sxz.size())), _L->_par.sxz, _L->_par.rxz, _L->_par.dwidthx, _L->_par.dwidthz, _L->_par.dfloor, _L->_par.dpower, _L->_par.xextension, _L->_par.zextension, true, true);
