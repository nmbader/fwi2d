...
protected:
    ...
    std::shared_ptr<vec> _W; // normalized weights for the extended model
...
public:
    ...
    sWeighting(const hypercube<data_t> &domain,const std::vector<std::vector<data_t> > &sxz, const std::vector<std::vector<std::vector<data_t> > > &rxz, const data_t dwidthx, const data_t dwidthz, const data_t dpower, const data_t xextension, const data_t zextension){
        ...
        _dpower = dpower;
        _sxz=sxz;
        _W = std::make_shared<vec>(hyper(axes[0],axes[1],S));
        _W->set(1.0);

        // normalization factors (sum of _W over shots)
        std::shared_ptr<vec> nW = std::make_shared<vec>(hyper(axes[0],axes[1]));
        nW->zero();
        ...
        for (int s=0; s<ns; s++){
            ...
            //    data_t val0x=val0;
            //    for (int iz=0; iz<v[2]-izw; iz++) pw[s][ix][iz] = val0x*val0;
            //    for (int iz=std::max(v[2]-izw,0); iz<v[2]; iz++) pw[s][ix][iz] = val0x*pow(cos(_dpower*0.5*M_PI*(v[2]-iz)/izw),2);
            //    for (int iz=v[2]; iz<v[3]; iz++) pw[s][ix][iz] = val0x;
            //    for (int iz=v[3]; iz<std::min(v[3]+izw,Z.n); iz++) pw[s][ix][iz] = val0x*pow(cos(_dpower*0.5*M_PI*(v[3]-iz)/izw),2);
            //    for (int iz=v[3]+izw; iz<Z.n; iz++) pw[s][ix][iz] = val0x*val0;
            //}
            ...
            for (int ix=v[0]; ix<v[1]; ix++){
                ...
                //for (int iz=0; iz<v[2]-izw; iz++) pw[s][ix][iz] = val0x*val0;
                for (int iz=std::max(v[2]-izw,0); iz<v[2]; iz++) pw[s][ix][iz] = val0x*pow(cos(_dpower*0.5*M_PI*(v[2]-iz)/izw),2);
                for (int iz=v[2]; iz<v[3]; iz++) pw[s][ix][iz] = 1.0;
                for (int iz=v[3]; iz<std::min(v[3]+izw,Z.n); iz++) pw[s][ix][iz] = val0x*pow(cos(_dpower*0.5*M_PI*(v[3]-iz)/izw),2);
                //for (int iz=v[3]+izw; iz<Z.n; iz++) pw[s][ix][iz] = val0x*val0;
            }
        ...
        for (int s=0; s<ns; s++){
            for (int ix=0; ix<X.n; ix++){
                for (int iz=0; iz<Z.n; iz++) pnw[ix][iz] += pw[s][ix][iz];
            }
        }
        ...
        for (int s=0; s<ns; s++){
            for (int ix=0; ix<X.n; ix++){
                for (int iz=0; iz<Z.n; iz++) pw[s][ix][iz] /= pnw[ix][iz];
            }
        }
    }
    ...
    std::shared_ptr<vec> getW(){return _W;}
...
protected:    
    std::shared_ptr<vec> _W; // weights

public:
    sWSum(){}
    ~sWSum(){}
    sWSum(const hypercube<data_t> &domain, std::shared_ptr<vec> W){
        successCheck(domain.getNdim()>=2,__FILE__,__LINE__,"The domain must contain at least 2 axes\n");
        _domain = domain;
        std::vector<ax > axes = domain.getAxes();
        axes.pop_back();
...
///WARNING: shallow copy of the weights
        _W = W;
    }
    sWSum * clone() const {
...
public:
    ...
    model_extension(sWeighting * W){
        ...
        _domain = *W->getDomain();
        _range = _domain;
        _wS = new sWSum(*W->getRange(),W->getW());
