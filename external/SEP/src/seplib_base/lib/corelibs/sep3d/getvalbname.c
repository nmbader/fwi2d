...
AUTHOR
   SEP , July ... 1995
   Revised: Stewart A. Levin (SEP)  8/8/2015
            Return error code on invalid seeks, don't exit!
...
_XFUNCPROTOBEGIN
int sep_get_val_by_name(char *tag_history, int *record_number,                                         char *key_name, int *n_values, void *values)
_XFUNCPROTOEND
#else 
int sep_get_val_by_name(tag_history, record_number, key_name, n_values, values)
char *tag_history;
int  *record_number;
char *key_name;
int *n_values;
...
#endif
{
    ...
    for (i_values =0; i_values < *n_values ; i_values++) {
	    ...
	    return (-1) /*seperr("sseek error seek_byte=%d\n",seek_byte)*/;
	}
